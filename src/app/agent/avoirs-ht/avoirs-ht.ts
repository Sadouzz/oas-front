...
@Component({
  ...
  templateUrl: './avoirs-ht.html',
})
export class AvoirsHt extends BasePaginatedComponent implements OnInit {
  ...
  save() {
    ...
    this.service.create({
      ...
      next: () => {
        this.saving = false;
        this.showSuccess('Avoir note de prix (HT) créé avec succès ! Les pièces ont été réintégrées en stock.');
        this.closeCreate();
        this.load();
      },
      error: (err: any) => {
        ...
        this.errorMessage = msg || 'Erreur lors de la création de l\'avoir HT.';
      }
    });
  }

  openDetail(a: AvoirHT) {
  ...
  downloadPdf(a: AvoirHT) {
    this.service.downloadPdf(a.id).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const anchor = document.createElement('a');
        anchor.href = url;
        anchor.download = `AvoirHT_${a.numero || a.id}.pdf`;
        anchor.click();
